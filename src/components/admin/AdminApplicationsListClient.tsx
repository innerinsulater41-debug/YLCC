          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-[#EFE8DD]">
              {filtered.length === 0 ? (
                ...
                filtered.map((app) => (
                  <tr key={app.id} className="hover:bg-[#FAF7F0]/60 transition-colors">
                    ...
                    <td className="p-4 text-[#57534E]">
                      <span className="font-semibold text-[#8C6527]">{app.phone}</span>
                      <span className="block text-[10px] text-[#78716C]">{app.email}</span>
                    </td>

                    <td className="p-4 text-[#192538] font-medium">
                    ...
                    <td className="p-4">
                      {app.documentUrls && app.documentUrls.length > 0 ? (
                        ...
                        <span className="text-[#A89577]">None</span>
                      )}
                    </td>
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="px-2.5 py-1 rounded bg-[#8C6527] text-white hover:bg-[#74511D] font-bold text-[11px]"
                        >
                          Dossier
                        </button>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 sm:p-8 space-y-6 shadow-2xl border border-[#D4C5AD]">
            <div className="flex items-center justify-between border-b border-[#EFE8DD] pb-3">
              <div>
                <span className="text-[10px] uppercase font-bold text-[#8C6527]">
                  Admission Dossier #{activeApp.id}
                </span>
                <h3 className="text-xl font-serif font-bold text-[#192538]">
            ...
            <div className="grid grid-cols-2 gap-4 text-xs">
              <div>
                ...
                <a href={`tel:${activeApp.phone}`} className="font-bold text-[#8C6527]">
                  {activeApp.phone}
                </a>
              </div>
              <div>
                <span className="text-[#78716C] block">Email:</span>
                <span className="font-semibold text-[#192538]">{activeApp.email}</span>
              </div>
              <div>
                <span className="text-[#78716C] block">Date of Birth:</span>
                <span className="font-semibold text-[#192538]">{activeApp.dob}</span>
              </div>
              <div>
                <span className="text-[#78716C] block">Qualification:</span>
                <span className="font-semibold text-[#192538]">{activeApp.qualification}</span>
              </div>
              <div>
                <span className="text-[#78716C] block">Selected Program:</span>
                <span className="font-bold text-[#8C6527]">{activeApp.selectedProgram}</span>
              </div>
              <div>
                <span className="text-[#78716C] block">Learning Mode & Batch:</span>
                <span className="font-semibold text-[#192538]">
                  {activeApp.preferredMode} ({activeApp.preferredBatch})
                </span>
              </div>
            </div>

            <div className="text-xs space-y-1 bg-[#FAF7F0] p-3 rounded-xl border border-[#E2D7C3]">
              <span className="font-bold text-[#192538]">Residential Address:</span>
              <p className="text-[#57534E]">{activeApp.address}</p>
            </div>

            {activeApp.careerGoal && (
              <div className="text-xs space-y-1 bg-[#FAF7F0] p-3 rounded-xl border border-[#E2D7C3]">
                <span className="font-bold text-[#192538]">Career Aspirations / Expectation:</span>
            ...
            {activeApp.documentUrls && activeApp.documentUrls.length > 0 && (
              <div className="space-y-2">
                <span className="text-xs font-bold text-[#192538] block">Attached Documents:</span>
                <div className="flex flex-wrap gap-2">
                  {activeApp.documentUrls.map((url, i) => (
                    <a
                      ...
                      className="p-2 rounded-lg bg-[#FAF7F0] border border-[#E2D7C3] text-xs font-semibold text-[#8C6527] flex items-center gap-1.5 hover:bg-[#ECE4D4]"
                    >
            ...
            <div className="space-y-2 pt-2 border-t border-[#EFE8DD]">
              <label className="block text-xs font-bold text-[#192538]">
              ...
              <textarea
                ...
                className="w-full px-3 py-2 text-xs bg-[#FAF7F0] border border-[#D4C5AD] rounded-xl text-[#192538]"
              />
              <div className="flex justify-end">
                <button
                  ...
                  className="bg-[#192538] hover:bg-[#0F172A] text-white px-4 py-1.5 rounded-lg text-xs font-bold transition-colors"
                >
